...
import { getAdminCounts, getCustomAdminNav, getCustomSectionCounts, getInboxUnreadCount } from "@/lib/portfolio";

export default async function AdminHomePage() {
  const [counts, customNav, customCounts, unreadCount] = await Promise.all([
    getAdminCounts(),
    getCustomAdminNav(),
      ...
      <div className="grid gap-4 sm:grid-cols-2 xl:grid-cols-3">
        <Link href="/admin/inbox">
          <Card className="p-6 transition-colors hover:ring-accent/40">
            <p className="text-[11px] tracking-[0.2em] text-muted uppercase">Inbox unread</p>
            <p className="font-display mt-3 text-4xl tracking-tight">{unreadCount}</p>
          </Card>
        </Link>
